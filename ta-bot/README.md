...
Просто `/start` в чате с ботом — дальше всё на кнопках: выбираешь символ (BTCUSDT/ETHUSDT/SOLUSDT или EUR-USD/GBP-USD/USD-JPY), потом таймфрейм — бот сам присылает график и сигнал.

Для точечных запросов вне списка кнопок остаётся текстовая команда:
```
