...
if(-not (Test-Path $dest)){ Write-Host ('Folder not found: '+$dest); Write-Host 'Run the download .bat first.'; return }
...
$rawsum=363
$audio=@{ ru=@{parts=@('ru-1.mp3','ru-2.mp3','ru-3.mp3','ru-4.mp3','ru-5.mp3','ru-6.mp3');dur=462.08}; en=@{parts=@('en-1.mp3','en-2.mp3','en-3.mp3','en-4.mp3','en-5.mp3');dur=412.08} }
foreach($lang in @('ru','en')){
  $parts=$audio[$lang].parts; $adur=$audio[$lang].dur
  $missing=$parts | Where-Object { -not (Test-Path $_) }
  if($missing){ Write-Host ('['+$lang+'] skip - missing: '+($missing -join ', ')); continue }
  ...
  $out=$lang+'.mp4'
  & $ff -y -f concat -safe 0 -i $ilist -i $full -vf 'scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:white,fps=25,format=yuv420p' -c:v libx264 -pix_fmt yuv420p -c:a aac -b:a 192k -shortest $out
  Write-Host ('['+$lang+'] done -> '+$out)
}
Write-Host 'ALL DONE. ru.mp4 and en.mp4 are in the folder.'
